@echo off
REM Build InfinityHost (Release x64) -> InfinityHost.dll + InfinityHost.lib.
...
"!MSBUILD!" "%~dp0InfinityHost.vcxproj" -p:Configuration=Release -p:Platform=x64 -p:SolutionDir=%~dp0..\ -m -nologo
if errorlevel 1 (
  echo [build] FAILED
  exit /b 1
)
echo [build] OK -^> %~dp0..\x64\Release\Plugins\InfinityHost.dll ^(+ InfinityHost.lib^)
endlocal
