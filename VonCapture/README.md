...
Files are named by **player** — `speaker_<name>_<steam64>.opus` — using identity
records the plugin writes on connect (the VoN speaker id is bound to the
player's SteamID + name). A speaker with no identity record falls back to
`speaker_<vonId>.opus`. `timeline.csv` includes `steam64` and `name` columns.
